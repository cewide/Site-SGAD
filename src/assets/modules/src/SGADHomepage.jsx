...
function ModuleDetailModal({ module, onClose }) {
  ...
  useEffect(() => {
    ...
    return () => {
      el.removeEventListener("close", syncParent);
      if (el.open) el.close();
    };
  }, [onClose]);
  ...
  return (
    <dialog
      ref={dialogRef}
      className="sgad-module-dialog text-white"
      aria-modal="true"
      aria-labelledby="module-detail-title"
    >
      <div
        className="flex min-h-[100dvh] w-full items-end justify-center bg-transparent p-4 sm:items-center sm:p-6"
        onClick={(e) => {
      ...
      >
        <div
          role="document"
          className="relative z-10 flex max-h-[min(92vh,880px)] w-full max-w-4xl flex-col overflow-hidden rounded-t-2xl border border-white/10 bg-slate-900 shadow-2xl sm:rounded-2xl"
        ...
        >
          ...
          <button
            ...
            className="absolute right-4 top-4 z-20 flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-slate-800/90 text-white transition-colors hover:bg-slate-700"
            aria-label="Fechar"
          >
            <X size={20} />
          </button>

          <div className="max-h-[inherit] min-h-0 flex-1 overflow-y-auto overscroll-contain pt-1">
            <div className="relative w-full border-b border-white/[0.06] bg-slate-950">
              <img
                src={module.image}
                alt={`Interface do módulo ${module.name} no SGAD`}
                className="mx-auto block h-auto max-h-[min(50vh,420px)] w-full object-contain object-top sm:max-h-[min(55vh,480px)]"
                draggable={false}
              />
            </div>
            <div className="p-6 pb-8 sm:p-8">
              <h2 id="module-detail-title" className="mb-4 pr-12 text-2xl font-bold text-white sm:text-3xl">
                {module.name}
              </h2>
              <p className="text-base leading-relaxed text-slate-300">{module.desc}</p>
              <p className="mt-6 border-t border-white/[0.06] pt-6 text-sm text-slate-500">
                Captura de tela real do SGAD. Clique fora do painel ou pressione Esc para fechar.
              </p>
            </div>
          </div>
        </div>
          ...
          {modules.map((m) => (
            <div key={m.name} className="h-full min-h-0">
              <button
                ...
                className="group relative z-0 flex min-h-[280px] w-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.03] text-left transition-all duration-300 hover:border-white/[0.15] hover:bg-white/[0.06] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 active:scale-[0.99]"
              >
